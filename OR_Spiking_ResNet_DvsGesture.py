...
class ResNet18(nn.Module):
    def __init__(self):
        ...
        self.first_block = nn.Sequential(
            layer.Conv2d(2, 64, kernel_size=7, stride=2, padding=3, bias=False),
...
parser = argparse.ArgumentParser(description='spikingjelly LIF DVS128 Training')
...
parser.add_argument('-b', '--batch-size', default=32, type=int, help='Batch\n')
parser.add_argument('-T', '--timesteps', default=32, type=int, dest='T', help='Simulation duration, such as 100 \n')
parser.add_argument('--lr', '--learning-rate', default=1e-4, type=float, metavar='LR', help='Learning rate, such as "1e-3"\n', dest='lr')
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        for img, label in tqdm(train_loader):
            ...
            label_one_hot = F.one_hot(label, 11).float()            # Encode the label one hot for later loss calculation
        ...
        if dist.get_rank() == 0:
            ...
            #     torch.save(net.state_dict(), os.path.join(model_output_dir, 'or_t_t_attation.pt'))
